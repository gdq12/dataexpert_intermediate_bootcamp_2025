...
def get_tasks():
    ...
    if random_num:
        hash_string = str(random.randint(0, 1000000))
    user_id = str(hash(hash_string))
    color = statsig.get_experiment(StatsigUser(user_id), "lesson_button").get("Button Color", "green")
    paragraph_text = statsig.get_experiment(StatsigUser(user_id), "lesson_button").get("Paragraph Text", "Forest green")
